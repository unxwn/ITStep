...
function App() {
  ...
  useEffect(() => {
    fetch('http://192.168.1.48:5000/api/tasks') //!!!!!!! 192.168.1.48
      .then(response => response.json())
      .then(data => setTasks(data));

    fetch('http://192.168.1.48:5000/api/categories')
      .then(response => response.json())
  ...
  const addTask = (task) => {
    fetch('http://192.168.1.48:5000/api/tasks', {
      method: 'POST',
      headers: {
  ...
  const deleteTask = (id) => {
    fetch(`http://192.168.1.48:5000/api/tasks/${id}`, {
      method: 'DELETE'
    })
  ...
  const deleteCategory = (category) => {
    fetch(`http://192.168.1.48:5000/api/categories/${category}`, {
      method: 'DELETE'
    })
      .then(() => {
